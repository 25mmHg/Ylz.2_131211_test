 ...
 *
 * Eine kurze Novelle im Syntax der Programmiersprache C
 *
 *.....Yussuf Udue saß wie üblich Montags 7:32 in der Pariser Metro und befand sich auf dem Weg zur Arbeit. 
 * Neben Ihm nahm eine alte Dame Platz.
 * Sie klemmte sich zwischen ihn und das Fenster. Hinter dem rechten Ohr trug sie ein Tatoo, das wie ein aufgefalteter
 * Regenschrm aussah. Er konnte nicht genau erkennen, was drunter geschrieben stand. "2nd age" war zu erkennen. Muß wohl
 * eine dieser seit etlichen Jahren um sich greifenden Weltuntergangssekten sein. Oder was mag diese Frau vor 40Jahren
 * bewegt haben? Sie zog einen alten Reader aus der Tasche hervor und wendete von diesem Moment an keinen Blick von dem
 * kleinen Display auf dem riesige Wörter erschienen. Wenn sich Yussuf nicht so gewaltig über den verlorenen Platz am 
 * Fenster geärgert hätte, dieses Bild war zum Schreien komisch. Aber so blieb ihm nur ein dicker
 * Kloß im Hals und das Knie dieser Frau. Jetzt verließ der Zug für einen Moment den Untergrund und bewegte sich über 
 * die neu gebaute Traverse du Seine. Die Brücke aus verzinktem Stahlrohr sah aus, wie eine plattgedrückte Achterbahn,
 * wenn man sie vom Ufer aus betrachtete. Er liebte diesen Anblick. Jetzt lag Schnee auf den Tragrohren, die über
 * ihm entlangliefen und Yussuf schaute in das schwarze Wasser. Die Sonne stand tief und blendete.
 * Die alte Dame sah kurz auf und drehte den Kopf zum Fenster. Jetzt konnte Yussuf das Etwas, das immer noch wie ein
 * Regenschirm aussah, gut erkennen. Hinten an dem Regenschirm war ein sechseckiger Zylinder befestigt. Und etwas, das wie eine
 * große planare Antenne aussah.....
 *
 *
 * Was war passiert?
 *
 ...
 * Beschleunigungsantrieb für interstellare Missionen erörtern sollte. Bei dem Zwischenfall kam es während einer 
...
/*
 * Erzeugung einer Zufallszahl mit Hilfe des Quantisierungsrauschens bei Analog-Digital Wandlungen:
 */
uint16_t get_analog_random()
{
    uint8_t zahl2=0, zahl=0;
...
/*
 * Erzeugung einer 1MHz RF-Trägerwelle:
 *
 * Schaltung:
 *  
 *  
 ...
 *
 *
 *        (created with AACircuit v1.28.6 beta 04/19/05 www.tech-chat.de)
 */
 ...
 * transportiert werden kann. Um eine oberwellenreiche perkussive Schwingung zu erreichen wird hierbei mit einer 
...
int main(void)
{
    ...
    _delay_ms(200); // Verzögerung nach Reset. Kurze Reset-Impulse reichen für vollständigen Stop der Emission aus.
    ...
    {        
        ...
        do
        {
            for (uint8_t i=0; i<16; i++)
            {
                ...
                /*
                 * Bei der oben aufgeführten Zeile handelt es sich um ein in Software realisiertes 16Bit breites
                 * linear rückgekoppeltes Schieberegister (LFSR) mit intern realisierter XOR-Verknüpfung.
                 ...
                 * Bauchschuss der ihm während eines Duells zugefügt wurde. Am Vorabend des Duells brachte Galois seine
                 * mathematische Theorie in einem langen Brief zu Papier. Erst Jahre später wurde die Bedeutung dieser
                 * Zeilen erkannt. Heute stellt die Galoistheorie eine fundamentale Theorie der linearen Algebra dar und
                 * spielt eine wichtige Rolle bei der Lösung quadratischer Gleichungen mit mehreren Polynomen.
                 * Die Codierung von Algorithmen zur Erzeugung von Maximalfolgen mit linearen Schieberegistern lässt sich
                 * bei Verwendung des Ansatzes nach E.Galois stark vereinfachen und geschieht hier in einer einzigen Zeile.
                 */
                lfsr_out<<=1;
            ...
            {
                /*
                 * Auslesen eines Potentiometers. 64 unterschiedliche Stellungen werden erkannt und bewirken Variationen der
                 * Pulsfolgen zur Erzeugung der Knacklaute des Geigerzählers.
                 */
                uint8_t poti=0;             
 ...
 * Dies ist für eine authentische Simulation notwendig, da es auch beim radioaktivem Zerfall keinen Träger gibt.
 *
